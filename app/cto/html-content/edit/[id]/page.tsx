...
export default function CTOEditHTMLContent({ params }: PageProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        router.push(`/cto/html-content/view/${content.id}`)
      } else {
  ...
  const categories = ['Technical Strategy Documents', 'Infrastructure & DevOps', 'AI & Machine Learning', 'Team & Process Management']
        ...
        <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="flex gap-4 mt-8">
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white py-2 rounded-lg font-semibold transition-colors flex items-center justify-center"
            >
              {saving ? (
                <>

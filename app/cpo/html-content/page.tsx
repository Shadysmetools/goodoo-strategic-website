...
export default function CPOHTMLContent() {
  ...
  const [newContent, setNewContent] = useState({
    title: '',
    content: '',
    category: 'Product Strategy Documents',
    description: ''
  })
  ...
  const [categories, setCategories] = useState(['Product Strategy Documents', 'Market & Competitive Analysis', 'Product Development', 'Go-to-Market Materials'])
  ...
  const loadCategories = () => {
    const savedCategories = localStorage.getItem('cpo-categories')
    if (savedCategories) {
  ...
  const saveCategories = (newCategories: string[]) => {
    ...
    localStorage.setItem('cpo-categories', JSON.stringify(newCategories))
  }
  ...
  const deleteCategory = (index: number) => {
    if (confirm('Are you sure you want to delete this category? Content in this category will be moved to "Product Strategy Documents".')) {
      const categoryToDelete = categories[index]
      const updatedCategories = categories.filter((_, i) => i !== index)
      saveCategories(updatedCategories)
      
      // Move content from deleted category to "Product Strategy Documents"
      const updatedContents = contents.map(content => 
        content.category === categoryToDelete 
          ? { ...content, category: 'Product Strategy Documents' }
          : content
      )
      setContents(updatedContents)
      
      // Update content in storage
      localStorage.setItem('cpo-html-content', JSON.stringify(updatedContents))
    }
  }
  ...
  const fetchContents = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/html-content?role=cpo')
      if (response.ok) {
  ...
  const handleCreateContent = async () => {
    try {
      ...
      if (response.ok) {
        await fetchContents()
        setNewContent({ title: '', content: '', category: 'Product Strategy Documents', description: '' })
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">CPO HTML Content Management</h1>
            <p className="text-xl text-gray-600">Create and manage HTML webpages for product content</p>
          </div>
          ...
          <div className="flex flex-wrap gap-4 mb-8 justify-center">
            <button
              ...
              className="bg-pink-600 hover:bg-pink-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors duration-200 flex items-center space-x-2"
            >
              <span>📝</span>
              ...
              <div key={category} className="bg-white rounded-xl shadow-lg p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {categoryContents.map((content) => (
                    <div
                      key={content.id}
                      className="group border border-gray-200 rounded-lg p-6 hover:shadow-lg transition-all duration-200 hover:border-pink-300"
                    >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-2 items-center mt-2">
                            ...
                            <button
                              ...
                              onClick={() => {
                                navigator.clipboard.writeText(`${window.location.origin}/cpo/html-content/shared/${content.shareToken}`)
                              }}
                            >
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded-lg text-sm font-medium transition-colors"
                  >
                    + New
                  </button>
                </div>

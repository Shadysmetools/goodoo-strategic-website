...
export default function CEOEditHTMLContent({ params }: PageProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        router.push(`/ceo/html-content/view/${content.id}`)
      } else {
  ...
  const categories = ['Strategic Documents', 'Financial Documents', 'Market Research', 'Board & Investor Materials']
        ...
        <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="flex gap-4 mt-8">
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white py-2 rounded-lg font-semibold transition-colors flex items-center justify-center"
            >
              {saving ? (
                <>

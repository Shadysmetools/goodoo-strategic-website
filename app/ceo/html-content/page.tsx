...
export default function CEOHTMLContent() {
  ...
  const [categories, setCategories] = useState(['Strategic Documents', 'Financial Documents', 'Market Research', 'Board & Investor Materials'])
  ...
  const loadCategories = () => {
    const savedCategories = localStorage.getItem('ceo-categories')
    if (savedCategories) {
  ...
  const saveCategories = (newCategories: string[]) => {
    ...
    localStorage.setItem('ceo-categories', JSON.stringify(newCategories))
  }
  ...
  const deleteCategory = (index: number) => {
    if (confirm('Are you sure you want to delete this category? Content in this category will be moved to "Strategic Documents".')) {
      const categoryToDelete = categories[index]
      const updatedCategories = categories.filter((_, i) => i !== index)
      saveCategories(updatedCategories)
      
      // Move content from deleted category to "Strategic Documents"
      const updatedContents = contents.map(content => 
        content.category === categoryToDelete 
          ? { ...content, category: 'Strategic Documents' }
          : content
      )
      setContents(updatedContents)
      
      // Update content in storage
      localStorage.setItem('ceo-html-content', JSON.stringify(updatedContents))
    }
  }
  ...
  const fetchContents = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/html-content?role=ceo')
      if (response.ok) {
  ...
  const handleCreateContent = async () => {
    try {
      ...
      if (response.ok) {
        await fetchContents()
        setNewContent({ title: '', content: '', category: 'Strategic Documents', description: '' })
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h1 className="text-4xl font-bold text-gray-900 mb-4">CEO HTML Content Management</h1>
            <p className="text-xl text-gray-600">Create and manage HTML webpages for strategic content</p>
          </div>
                    ...
                    >
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-2">
                            <Link
                              href={`/ceo/html-content/view/${content.id}`}
                          ...
                          <div className="flex gap-2 items-center mt-2">
                            ...
                            <button
                              ...
                              onClick={() => {
                                navigator.clipboard.writeText(`${window.location.origin}/ceo/html-content/shared/${content.shareToken}`)
                              }}
                            >
